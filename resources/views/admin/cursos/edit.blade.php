        ...
        <form action="{{ route('admin.cursos.update', $curso) }}" method="POST" class="space-y-4 bg-white p-6 rounded shadow">
            ...
            <div>
                ...
                <textarea name="descripcion" rows="3"
                          class="mt-1 w-full border-gray-300 rounded shadow-sm focus:ring-blue-500">{{ old('descripcion', $curso->descripcion) }}</textarea>
            ...
            <div>
                ...
                <input type="number" name="horas" value="{{ old('horas', $curso->horas) }}" min="1"
                       class="mt-1 w-full border-gray-300 rounded shadow-sm focus:ring-blue-500">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="fecha_inicio" value="{{ old('fecha_inicio', $curso->fecha_inicio ? \Carbon\Carbon::parse($curso->fecha_inicio)->format('Y-m-d') : '') }}"
                           class="mt-1 w-full border-gray-300 rounded shadow-sm focus:ring-blue-500">
                    @error('fecha_inicio') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">Fecha de Fin</label>
                    <input type="date" name="fecha_fin" value="{{ old('fecha_fin', $curso->fecha_fin ? \Carbon\Carbon::parse($curso->fecha_fin)->format('Y-m-d') : '') }}"
                           class="mt-1 w-full border-gray-300 rounded shadow-sm focus:ring-blue-500">
