    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.diplomas.mass.download-combined', $curso) }}"
                   class="px-3 py-1.5 bg-blue-600 text-white rounded text-sm hover:bg-blue-700">
                    PDF completo (todos)
                </a>
                <form action="{{ route('admin.diplomas.mass.regenerate', $curso) }}" method="POST" class="inline" onsubmit="return confirm('¿Eliminar todos los diplomas de este curso y regenerarlos?')">
                    @csrf
                    <button type="submit" class="px-3 py-1.5 bg-yellow-500 text-white rounded text-sm hover:bg-yellow-600">
                        Regenerar
                    </button>
                </form>
