        ...
        <form method="POST" action="{{ route('admin.diplomas.mass.store') }}" class="bg-white shadow rounded-lg p-6 space-y-4">
            ...
            <div>
                ...
                @if($templatePreseleccionado)
                    ...
                    <div class="w-full border rounded px-3 py-2 bg-gray-50 text-gray-700">
                        {{ $templatePreseleccionado->nombre }}
                    </div>
                @else
            ...
            <div class="bg-amber-50 border border-amber-200 rounded p-3 text-sm text-amber-800">
                <strong>Nota:</strong> Se generarán diplomas para todos los alumnos inscritos
                en el curso seleccionado. Los alumnos que ya tengan un diploma
                para este curso serán omitidos.
            </div>
