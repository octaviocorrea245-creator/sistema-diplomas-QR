<x-app-layout>
    ...
    @php $xFirmar = old('firmante_id') ? 'true' : ($firmantes->count() === 1 ? 'true' : 'false'); @endphp
        ...
        <div class="form-card">
            <form method="POST" action="{{ route('admin.diplomas.mass.store') }}" id="mass-form"
                ...
                <div class="form-group">
                    ...
                    @else
                        <select name="curso_id" id="cursoSelect" required class="form-select">
                ...
                @else
                    <input type="hidden" name="template_id" id="templateSelect" value="">
                @endif
                ...
                @elseif($firmantes->count() === 1)

                    @php $f = $firmantes->first(); @endphp
                    <input type="hidden" name="firmante_id" value="{{ $f->id }}">
                    <div class="firma-box">
                        <div class="section-label">Firmante</div>
                        <div class="field-disabled" style="margin-bottom:1rem;">{{ $f->nombre }} — {{ $f->cargo }}</div>
                        <div class="form-group" style="margin-bottom:0;">
                            <label class="form-label">Contraseña de llave privada (.key)</label>
                            <input type="password" name="password_firma" class="form-input" required
                    ...
                    <div style="background:#fff;border-radius:16px;padding:2rem;max-width:420px;width:90%;box-shadow:0 20px 60px rgba(0,0,0,0.2);"
                        ...
                        <p style="font-size:0.875rem;color:#64748b;margin-bottom:1.25rem;">
                            ...
                            @if($firmantes->count() === 1)
                                <span style="color:var(--brand);font-weight:600;">Se aplicará firma electrónica a cada PDF.</span>
                            @else
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var templateInput = document.getElementById('templateSelect');
            if (!cursoSelect || !templateInput) return;

            var templates = @json($templates->map(fn($t) => ['id' => $t->id, 'curso_id' => $t->curso_id]));

            function autoSelectTemplate() {
                var cursoId = cursoSelect.value;
                if (!cursoId) return;

                var match = templates.find(function(t) { return t.curso_id == cursoId; });
                if (match) {
                    templateInput.value = match.id;
                } else {
                    templateInput.value = '';
                    if (typeof showToast === 'function') {
                        showToast('Este curso no tiene plantilla de diploma.', 'warning');
                    }
                }
            }

            cursoSelect.addEventListener('change', autoSelectTemplate);
